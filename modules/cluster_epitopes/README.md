...
| File | Responsibility |
|---|---|
| `step.py` | `ClusterEpitopesStep` orchestration: `run` and `describe_outputs` |
...
| `__init__.py` | Facade that re-exports `ClusterEpitopesStep` |

`core.py` is pure compute (NetworkX, NumPy, Biopython `PairwiseAligner`) with no XLSX and no prompts.
...
| `clusters/CLUSTER_{track_id}.xlsx` | Same rows sorted by `cluster_id`, colour-coded (see below). |
| `clusters/CLUSTER_VIEW_{track_id}.csv` | Slim per-step view: `peptide, cluster_id, cluster_size, cluster_method`. |
...
| Colour | Hex | Meaning |
|---|---|---|
| Light grey | `D3D3D3` | Header row |
| White / light blue | `FFFFFF` / `EAF3FF` | Alternating bands, one shade per cluster, so adjacent clusters are easy to tell apart |
| Light orange | `FFE4B5` | `cluster_id` column |
| Pink | `FFB6C1` | `cluster_size` column |
...
- `cluster_threshold`: identity cutoff (0.0 to 1.0, default 0.8)
- `cluster_method`: `cluster_break`, `single_linkage`, or `clique` (default `cluster_break`)
