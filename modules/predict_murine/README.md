...
Re-runs the two MHC-I binding predictors with H-2 (murine) alleles against the ★ representatives so the human-selected epitopes can be ranked by how well they translate to mouse models. **It is qualitative: no peptides are removed.**
...
`curate_murine` joins this murine annotation onto each ★ epitope, so candidates that are both strong human binders and translatable to mouse strains are easy to spot for in vivo validation.
...
| `__init__.py` | Facade that re-exports `PredictMurineStep` |
...
|---|---|---|
| `default` | H-2-Db, H-2-Kb, H-2-Dd, H-2-Kd, H-2-Ld (5) | C57BL/6 + BALB/c together; the menu default |
| `C57BL/6` | H-2-Db, H-2-Kb (2) | Most common immunology lab strain |
| `BALB/c` | H-2-Dd, H-2-Kd, H-2-Ld (3) | Th2-biased strain, very common for vaccine work |
| `CBA/C3H/AKR` | H-2-Dk, H-2-Kk (2) | The H-2k haplotype shared by these strains |
| `all` | the 7 alleles above, consolidated | Maximum coverage, slower |

The groups are defined in `config.MURINE_ALLELES`. The chosen group is asked once and saved to `project_config["murine_strain"]`; on a rerun the saved choice is offered for editing.
...
- Pick the strain group your wet lab actually uses; running `all` is heavier (more API calls and more MHCFlurry batches) and rarely yields extra useful candidates.
...
- Input is peptide-direct, not from FASTA, so no length re-enumeration happens. If you want to test additional lengths, rerun `predict_binding` with a wider `peptide_lengths` first.
