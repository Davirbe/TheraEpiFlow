...
> **Code layout:** single-file module (`__init__.py`), small enough that the per-role split (see the top-level README) would only fragment it. It would split along the standard seams (core/io/step) if it grew.
...
A **JOIN-only** step that does not re-rank, re-score or relabel anything. It
